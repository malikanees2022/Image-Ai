...
const AdminHamburgerTabs = ({show,setShow}) => {
    ...
    <div>
      {show && (
        <div className=" fixed bg-[#151419] h-screen  flex text-start text-[#d7d7d7]  flex-col gap-5 pt-10 pl-8 pr-40 rounded-r-2xl font-semibold justify-start">
            ...
            <Link to={'/submitai'} onClick={handleLinkClick}>
            <div className='flex gap-2 justify-start items-center'>
                    <IoHomeSharp/>
                    <h1>Home</h1>
                </div>
            </Link>
            <Link to={'/submitimage'} onClick={handleLinkClick}><div className='flex gap-2 justify-start items-center'>
                    <FaNewspaper/>
                    <h1>Submit Image</h1>
                </div></Link>
